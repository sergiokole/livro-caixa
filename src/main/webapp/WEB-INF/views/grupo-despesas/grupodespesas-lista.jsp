...
$(function(){
	...
	$('#adicionar').click(function(){
		document.location = '<c:url value="/grupo-despesas/novo" />'
	});
})
...
<div id="grid" >
	...
	<c:choose>
		<c:when test="${!empty lista}">
			<table>
				...
				<c:forEach var="vo" items="${lista}" >
					<c:url var="editar" value="/grupo-despesas/editar/${vo.id}" />
					<c:url var="remover" value="/grupo-despesas/remover/${vo.id}" />
					<tr>
		...
		<c:otherwise>
			<p>Não existem Grupo de Despesas, por favor adicionar!</p>
		</c:otherwise>
	</c:choose>
	<br /> 
</div>
