...
/*Table structure for table `canal_operacao_bancaria` */

DROP TABLE IF EXISTS `canal_operacao_bancaria`;

CREATE TABLE `canal_operacao_bancaria` (
  `id` smallint(6) NOT NULL AUTO_INCREMENT,
  `nome` varchar(255) NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=latin1;

/*Data for the table `canal_operacao_bancaria` */

insert  into `canal_operacao_bancaria`(`id`,`nome`) values (1,'InternetBaking'),(2,'App Celular'),(3,'Caixa Presencial'),(4,'Caixa Eletrônico');
...
CREATE TABLE `conta_bancaria` (
  ...
  `canal_operacao_bancaria_id` smallint(6) DEFAULT NULL,
  `data` date NOT NULL,
  ...
  KEY `FK_CANAL_OPERACAO_BANCARIA` (`canal_operacao_bancaria_id`),
  ...
  CONSTRAINT `FK_CANAL_OPERACAO_BANCARIA` FOREIGN KEY (`canal_operacao_bancaria_id`) REFERENCES `canal_operacao_bancaria` (`id`),
...
CREATE TABLE `operacao_bancaria_por_canal` (
  `operacao_bancaria_id` smallint(6) NOT NULL,
  `canal_operacao_bancaria_id` smallint(6) DEFAULT NULL,
  UNIQUE KEY `IX_OPERACAO_BANCARIA_POR_CANAL` (`operacao_bancaria_id`,`canal_operacao_bancaria_id`),
  KEY `FK_CANAL_X_OPERACAO_BANCARIA` (`canal_operacao_bancaria_id`),
  CONSTRAINT `FK_CANAL_X_OPERACAO_BANCARIA` FOREIGN KEY (`canal_operacao_bancaria_id`) REFERENCES `canal_operacao_bancaria` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK_OPERACAO_BANCARIA_X_CANAL` FOREIGN KEY (`operacao_bancaria_id`) REFERENCES `operacao_bancaria` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
