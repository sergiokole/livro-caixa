...
CREATE TABLE `conta_bancaria` (
  ...
  `descricao` text,
  PRIMARY KEY (`id`),
  UNIQUE KEY `IX_OPERACAO_BANCARIA_POR_CANAL` (`operacao_bancaria_id`,`canal_atendimento_id`),
...
CREATE TABLE `operacao_bancaria_por_canal` (
  `operacao_bancaria_id` smallint(6) NOT NULL,
  `canal_atendimento_id` smallint(6) NOT NULL,
  `data` date DEFAULT NULL,
  PRIMARY KEY (`operacao_bancaria_id`,`canal_atendimento_id`),
  KEY `FK_CANAL_ATENDIMENTO_ID` (`canal_atendimento_id`),
  CONSTRAINT `FK_CANAL_ATENDIMENTO_ID` FOREIGN KEY (`canal_atendimento_id`) REFERENCES `canal_atendimento` (`id`),
  CONSTRAINT `FK_OPERACAO_BANCARIA_ID` FOREIGN KEY (`operacao_bancaria_id`) REFERENCES `operacao_bancaria` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
